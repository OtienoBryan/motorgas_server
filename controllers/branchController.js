...
const branchController = {
  ...
  createBranch: async (req, res) => {
    const { client_id, name, address, contact_person, phone, email } = req.body;
    
    try {
      const [result] = await db.query(
        `INSERT INTO branches (client_id, name, address, contact_person, phone, email)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [client_id, name, address, contact_person, phone, email]
      );
  ...
  updateBranch: async (req, res) => {
    ...
    try {
      ...
      await db.query(
        `UPDATE branches 
         SET name = ?, address = ?, contact_person = ?, phone = ?, email = ?
         WHERE id = ?`,
        [name, address, contact_person, phone, email, branchId]
      );
