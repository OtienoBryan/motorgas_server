...
const staffController = {
  getAllStaff: async (req, res) => {
    try {
      ...
      // Check the table structure and add missing columns we need (salary)
      const [columns] = await db.query('DESCRIBE staff');
      console.log('Staff table structure:', columns);
      const hasSalary = Array.isArray(columns) && columns.some(col => col.Field === 'salary');
      if (!hasSalary) {
        console.log('Adding missing salary column to staff table');
        await db.query('ALTER TABLE staff ADD COLUMN salary DECIMAL(12,2) NULL');
      }
  ...
  createStaff: async (req, res) => {
    const { name, photo_url, empl_no, id_no, role, station_id, role_id, phone } = req.body;
    
    try {
      const [result] = await db.query(
        'INSERT INTO staff (name, photo_url, empl_no, id_no, role, station_id, role_id, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [name, photo_url, empl_no, id_no, role, station_id, role_id, phone]
      );
  ...
  updateStaff: async (req, res) => {
    ...
    try {
      await db.query(
        'UPDATE staff SET name = ?, photo_url = ?, empl_no = ?, id_no = ?, role = ?, station_id = ?, role_id = ?, phone = ? WHERE id = ?',
        [name, photo_url, empl_no, id_no, role, station_id, role_id, phone, req.params.id]
      );
